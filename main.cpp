	...
	bool export_c_header )
{
	...
	// concat family and style names
	size_t sz_family = strlen( ft_face->family_name );
	size_t sz_style = strlen( ft_face->style_name );
	char *font_name = new char[sz_family + sz_style + 1]();
	strcpy( font_name, ft_face->family_name );
	strcat( font_name, " " );
	strcat( font_name, ft_face->style_name );
	...
	int font_size )
{
	...
	char *fn = new char[fn_size];
	sprintf( fn, "%s.sdf.png", orig_filename );
	printf( "'%s'\n", fn );
	LodePNG::Encoder encoder;
	...
	sprintf( fn, "%s.sdf.txt", orig_filename );
	FILE *fp = fopen( fn, "w" );
	if( fp ) {
